...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "POST") {
    try {
      ...
      // 성공적인 응답을 반환합니다.
      res.status(200).json({ message: "Supply created successfully.", supply: createdSupply });
    } catch (error) {
      // 요청이 실패하면 오류를 처리합니다.
    ...
    try {
      ...
      if (Name && Value) {
        if(Name ==="id"){
          let responseData = await prisma.supply.findMany({
            where: {
              [`${Name}`]: Number(Value),
            },
          });
          ...
          if(Value ==="TANK" || Value ==="CAN_DRUM"){ //탱크선택했을때 supply쪽
            let responseData = await prisma.supply.findMany({
              where: {
                OR:[
            ...
            let responseData = await prisma.supply.findMany({//그외를 선택하면 그거나 everyting
              where: {
                OR:[
                  {[`${Name}`]: Value},
                  {[`${Name}`]: "EVERYTHING"},
        ...
        else if(Name === "HIGH_LOW_PRICE" || Name === "SUPPROT_PASTE"){
          let responseData = await prisma.supply.findMany({
            where: {
              OR:[
                {[`${Name}`]: Value},
      ...
      //세로줄가져오기
        ...
        let responseData = await prisma.supply.groupBy({
    ...
    try {
      ...
      if (Name && Value) {
        ...
        if (typeof Value === "string") {
          const deleteResult = await prisma.supply.deleteMany({
            where: {
              [`${Name}`]: Value,
            },
          });
    ...
    try {
      ...
      const updatedSupply = await prisma.supply.updateMany({
        where: {
          [`${Name}`]: Value,
        },
        data: newData,
      ...
      res.status(200).json({ message: "supply updated successfully.", updatedRows: updatedSupply.count });
    } catch (error) {
