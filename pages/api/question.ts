...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "POST") {
    try {
      ...
      // 성공적인 응답을 반환합니다.
      res.status(200).json({ message: "question created successfully.", question: createdQuestion });
    } catch (error) {
      // 요청이 실패하면 오류를 처리합니다.
    ...
    try {
      ...
      if (Name && Value) {
        console.log("가로줄");
        if(Name==="id"){
          let responseData = await prisma.question.findMany({
            where: {
              [`${Name}`]: Number(Value),
            },
          });
      ...
      //세로줄가져오기
        ...
        let responseData = await prisma.question.groupBy({
    ...
    try {
      ...
      res.status(200).json({ message: "question updated successfully.", updatedRows: updatedQuestion.count });
    } catch (error) {
