...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "POST") {
    try {
      ...
      // 성공적인 응답을 반환합니다.
      res.status(200).json({ message: "application created successfully.", application: createdApplication });
    } catch (error) {
      // 요청이 실패하면 오류를 처리합니다.
    ...
    try {
      ...
      if (Name && Value) {
        if(Name ==="id"){
          let responseData = await prisma.application.findMany({
            where: {
              [`${Name}`]: Number(Value),
            },
          });
        ...
        else if(Name ==="FILLER_SUPPORT" || Name === "HIGH_LOW_PRICE" || Name==="ONE_COMPONENT_OR_TWO"){
          let responseData = await prisma.application.findMany({
            where: {
              OR:[
                {[`${Name}`]: Value},
        ...
        else if(Name ==="SUPPLY_FORM"){
          if(Value ==="TANK" || Value ==="CAN_DRUM"){ //탱크나 캔드럼선택했을때
            let responseData = await prisma.application.findMany({
              where: {
                OR:[
                  {[`${Name}`]: "TANK_OR_CAN_DRUM"},
                  {[`${Name}`]: Value},
                  {[`${Name}`]: "EVERYTHING"},
                ]
              },
            ...
            let responseData = await prisma.application.findMany({//그외를 선택하면 그거나 tank candrum제외나 everyting
              where: {
                OR:[
                  {[`${Name}`]: Value},
                  {[`${Name}`]: "EXCEPT_TANK_OR_CAN_DRUM"},
                  {[`${Name}`]: "EVERYTHING"},
                ]
              },
            });
      ...
      //세로줄가져오기
        ...
        let responseData = await prisma.application.groupBy({
    ...
    try {
      ...
      res.status(200).json({ message: "application updated successfully.", updatedRows: updatedApplication.count });
    } catch (error) {
