...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "POST") {
    try {
      ...
      // 성공적인 응답을 반환합니다.
      res.status(200).json({ message: "control created successfully.", control: createdControl });
    } catch (error) {
      // 요청이 실패하면 오류를 처리합니다.
    ...
    try {
      ...
      if (Name && Value) {
        if(Name ==="id"){
          let responseData = await prisma.control.findMany({
            where: {
              [`${Name}`]: Number(Value),
            },
          });
      ...
      //세로줄가져오기
        ...
        let responseData = await prisma.control.groupBy({
    ...
    try {
      ...
      const updatedControl = await prisma.control.updateMany({
        where: {
          [`${Name}`]: Value,
        },
        data: newData,
      ...
      res.status(200).json({ message: "control updated successfully.", updatedRows: updatedControl.count });
    } catch (error) {
